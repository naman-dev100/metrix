...
export async function DELETE(
  ...
  try {
    ...
    await query(`DELETE FROM "WorkoutSet" WHERE session_id = $1`, [id]);

    // Then delete the workout session (verify ownership)
    const result = await query(
      `DELETE FROM "WorkoutSession" WHERE id = $1 AND "userId" = $2 RETURNING id`,
      [id, session.user.id]
    );
